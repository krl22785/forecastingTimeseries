...
The series does not appear to be stationary.  The mean and the variance appears to change as time increases.  Furthermore, with the ACF gradually decaying and the first PACF is essentially 1, this apppears to be a random walk.  
...
The ACF and PACF for the residuals show a change in variance over time.  Furthermore, more than 5% of observations appear above the dotted line threshold, indicating some form of structure that is not being accounted for.  

The ACF and PACF for the residuals-squared show clusters of volatility, indicating that ARCH/GARCH might be best for this dataset.  
...
The p-values for all of the coefficients is less than .05, therefore indicating that they are all statistically signifcant.  Omega does seem to be particularly small in this formula.
...
The ARIMA-GARCH forecast is more specific versus the ARIMA model.  The forecast intervals deviate with the data instead of expanding like the ARIMA model when variance increases.  This ARIMA-GARCH model is definitely more specific then the basic ARIMA model at prediciting expected value ranges.  
...
The accuracy of this forecast model is very great.  The forecast range captures much of the structure in the log.rupee value.  The issue though is that it appears to have over-fit the exchange rate; for example, I would not feel comfortable using this to forecast because it is too accurate.  
...
Not too well.  Some of the long-tail seems to deviate significantly.
